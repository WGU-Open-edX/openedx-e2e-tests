...
class TestdocTest {
    ...
    async step(config, description, action, options) {
        let title;
        let desc;
        let act;
        let opts;
    ...
    async screenshot(config, description, options) {
        let title;
        let desc;
        let opts;
        if (typeof config === 'string') {
    ...
    async click(config, title, description, options) {
        let selector;
        let titleStr;
        let desc;
        let opts;
    ...
    async fill(config, value, title, description, options) {
        let selector;
        let val;
        let titleStr;
        let desc;
    ...
    async downloadFromHref(selector, downloadPath) {
    ...
    async uploadFile(selector, filePath, title, description) {
        ...
        const { stepNumber, numberedStepNumber, screenshot } = await this.highlight(selector, null, { title: title || `Upload file to ${selector}` });
    ...
    async uploadFileParagon(selector, filePath) {
        const absolutePath = path.isAbsolute(filePath) ? filePath : path.resolve(process.cwd(), filePath);
        try {
            // eslint-disable-next-line no-console
            console.log('Verifying file exists at path:', absolutePath);
        ...
        await this.highlight(selector, async () => {
    ...
    async ShowElement(selector, display) {
        await this.page.addStyleTag({
            content: `
          ${selector} {
