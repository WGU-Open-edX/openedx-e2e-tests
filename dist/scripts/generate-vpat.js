 ...
 */
function aggregateReports(reportsDir) {
 ...
 */
function generateVPATData(reportsDir, productInfo = {}) {
    const { criteriaMap: violationMap, pages } = aggregateReports(reportsDir);
    const wcagCriteria = WCAG_CRITERIA.map(criterion => {
...
function generateHTMLReport(data) {
    ...
    const renderCriteriaTable = (criteria) => criteria.map(c => {
        let statusClass = 'status-fail';
        if (c.conformanceLevel === 'Supports')
            statusClass = 'status-pass';
        else if (c.conformanceLevel === 'Not Evaluated')
 ...
 */
function generateRSTReport(data) {
    const renderCriteriaTable = (criteria, level) => {
